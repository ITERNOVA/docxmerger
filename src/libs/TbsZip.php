 ...
 * @license LGPL
 * @version TbsZip 2.15 - 20131016
 * @package iternova\docxmerger
 * @link https://www.tinybutstrong.com
 */
...
class TbsZip {
    ...
    function RaiseError( $Msg ) {
        if ( $this->DisplayError ) {
            if ( PHP_SAPI === 'cli' ) {
                echo get_class( $this ) . ' ERROR with the zip archive: ' . $Msg . PHP_EOL;
            } else {
                echo '<strong>' . get_class( $this ) . ' ERROR with the ZIP archive:</strong> ' . $Msg . '</br>' . PHP_EOL;
            }
        }
        $this->Error = $Msg;
        return false;
